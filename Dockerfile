...
RUN apt-get update && apt-get install -y --no-install-recommends \
        curl build-essential wget dpkg-dev && \
    # Install TA-Lib C library
    wget http://prdownloads.sourceforge.net/ta-lib/ta-lib-0.4.0-src.tar.gz && \
    tar -xzf ta-lib-0.4.0-src.tar.gz && \
    cd ta-lib && \
    ./configure --prefix=/usr --build="$(dpkg-architecture -q DEB_BUILD_GNU_TYPE)" && \
    make && \
    make install && \
...
# 3 - Copy the ready-made venv from builder
COPY --from=builder --chown=appuser:appuser /app/.venv /app/.venv

# 4 - Copy application source with correct ownership
COPY --chown=appuser:appuser . .

# 5 - Create required directories with proper ownership and permissions
...
# 6 - Entrypoint script and fix line endings
